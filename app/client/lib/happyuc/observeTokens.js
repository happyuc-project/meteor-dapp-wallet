...
var setupContractFilters = function(newDocument){
    ...
    var blockToCheckBack = (newDocument.checkpointBlock || 0) - happyucConfig.rollBackBy;

    // TODO change to 0, when new ghuc is out!!!!!
    ...
    filter.get(function(error, logs) {
        if(!error) {
            ...
            Tokens.update({_id: newDocument._id}, {$set: {
                checkpointBlock: (currentBlock || HucBlocks.latest.number) - happyucConfig.rollBackBy
            }});
        }
    });
    ...
    filter.watch(function(error, log){
        if(!error) {
            ...
            if(HucBlocks.latest.number && log.blockNumber > HucBlocks.latest.number) {
                // update last checkpoint block
                Tokens.update({_id: newDocument._id}, {$set: {
    ...
    collectionObservers[collectionObservers.length] = Tokens.find({}).observe({
        ...
        */
        added: function(newDocument) {
            ...
            webu.huc.getCode(newDocument.address, function(e, code) {
                if(!e) {
