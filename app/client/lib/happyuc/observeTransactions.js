...
addTransactionAfterSend = function(txHash, amount, from, to, gasPrice, estimatedGas, data, tokenId) {
    ...
    // add from Account
    HucAccounts.update({address: from}, {$addToSet: {
        transactions: txId
    }});

    // add to Account
    HucAccounts.update({address: to}, {$addToSet: {
        transactions: txId
    }});
};
...
addTransaction = function(log, from, to, value){
    ...
    var block = webu.huc.getBlock(log.blockNumber, false, function(err, block){
        if(!err) {

            webu.huc.getTransaction(log.transactionHash, function(err, transaction) {
                if(!err && transaction) {
                    webu.huc.getTransactionReceipt(log.transactionHash, function(err, receipt){
...
var updateTransaction = function(newDocument, transaction, receipt){
    ...
    if(newDocument.outOfGas) {
        ...
        if(HucAccounts.findOne({address: newDocument.from})) {
            webu.huc.getBalance(newDocument.from, newDocument.blockNumber, function(e, now){
                if(!e) {
                    webu.huc.getBalance(newDocument.from, newDocument.blockNumber-1, function(e, then){
    ...
    var checkTransactionConfirmations = function(tx){
        ...
        if(!tx.confirmed && tx.transactionHash) {
            ...
            var updateTransactions = function(e, blockHash){
                ...
                if(!e) {
                    var confirmations = (tx.blockNumber && HucBlocks.latest.number) ? (HucBlocks.latest.number + 1) - tx.blockNumber : 0;
                    confCount++;
                    ...
                    if(confirmations < happyucConfig.requiredConfirmations && confirmations >= 0) {
                        ...
                        webu.huc.getTransaction(tx.transactionHash, function(e, transaction){
                            webu.huc.getTransactionReceipt(tx.transactionHash, function(e, receipt){
                    ...
                    if(confirmations > happyucConfig.requiredConfirmations || confCount > happyucConfig.requiredConfirmations*2) {
                            ...
                            webu.huc.getTransactionReceipt(tx.transactionHash, function(e, receipt){
                                if(!e) {
                                    ...
                                    if(!transaction || !transaction.blockNumber) {
                                        ...
                                        webu.huc.getBlock(transaction.blockNumber, function(e, block) {
                                            if(!e) {
            ...
            var filter = webu.huc.filter('latest').watch(function(e, blockHash) {
    ...
    collectionObservers[collectionObservers.length] = Transactions.find({}).observe({
        ...
        */
        added: function(newDocument) {
            var confirmations = HucBlocks.latest.number - newDocument.blockNumber;
        ...
        */
        removed: function(document) {
            ...
            HucAccounts.update({address: document.from}, {$pull: {
                transactions: document._id
            }});
            HucAccounts.update({address: document.to}, {$pull: {
                transactions: document._id
            }});
        }
    });
