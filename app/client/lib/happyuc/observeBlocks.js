...
updateBalances = function() {
  ...
  _.each(creatingWallets, function(wallet) {
    ...
    webu.huc.getTransactionReceipt(wallet.transactionHash,
  ...
  // UPDATE ENS
  var allAccounts = HucAccounts.find().fetch().concat(walletsAndContracts);
    ...
    if (!account.ensCheck ||
      ...
      Helpers.getENSName(account.address, function(err, name, returnedAddr) {
        ...
        if (!err && account.address.toLowerCase() == returnedAddr) {
          HucAccounts.update({address: account.address},
              {$set: {name: name, ens: true, ensCheck: now}});
          CustomContracts.update({address: account.address},
              {$set: {name: name, ens: true, ensCheck: now}});
          Wallets.update({address: account.address},
              {$set: {name: name, ens: true, ensCheck: now}});
        } else {
          HucAccounts.update({address: account.address},
  ...
  var walletsContractsAndAccounts = HucAccounts.find().
      fetch().
  ...
  _.each(Tokens.find().fetch(), function(token) {
    ...
    _.each(walletsContractsAndAccounts, function(account) {
      tokenInstance.balanceOf(account.address, function(e, balance) {
        var currentBalance = (token && token.balances)
            ? token.balances[account._id]
            : 0;
...
observeLatestBlocks = function() {
  ...
  webu.huc.filter('latest').watch(function(e, res) {
    if (!e) {
      updateBalances();
    }
  });
