...
/**
 Config for the happyuc connector
...
happyucConfig = {
  /**
   ...
   @property happyucConfig.rollBackBy
   */
  rollBackBy: 0,
  /**
   Number of blocks to confirm a wallet

   @property happyucConfig.requiredConfirmations
   */
  requiredConfirmations: 12,
  /**
   The default daily limit used for simple accounts

   @property happyucConfig.dailyLimitDefault
   */
...
connectToNode = function() {
  ...
  if (HucAccounts.find().count() > 0) {
    checkForOriginalWallet();
  }

  // HucBlocks.detectFork(function(oldBlock, block){
  ...
  //         if(wallet.creationBlock + happyucConfig.requiredConfirmations >= block.number)
