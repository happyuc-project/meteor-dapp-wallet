...
var addCustomContract = function(e) {
  ...
  var address = $('.modals-add-custom-contract input[name="address"]').
          hasClass('dapp-error')
      ? ''
  ...
  if (webu.isAddress(address)) {
    ...
    CustomContracts.upsert({address: address}, {
      $set: {
        address: address,
        name: name,
    ...
    // update balances from lib/happyuc/observeBlocks.js
    updateBalances();
...
var addToken = function(e) {
  ...
  var address = $('.modals-add-token input[name="address"]').
          hasClass('dapp-error') ?
