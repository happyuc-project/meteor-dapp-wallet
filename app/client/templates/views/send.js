...
var checkOverDailyLimit = function(address, wei, template) {
  ...
  if (account && account.requiredSignatures > 1 &&
      !_.isUndefined(account.dailyLimit) && account.dailyLimit !==
      happyucConfig.dailyLimitDefault && Number(wei) !== 0) {
    // check whats left
    ...
    if (restDailyLimit.lt(new BigNumber(wei, 10)))
      TemplateVar.set('dailyLimitText', new Spacebars.SafeString(
          TAPi18n.__('wallet.send.texts.overDailyLimit', {
            limit: HucTools.formatBalance(restDailyLimit.toString(10)),
            total: HucTools.formatBalance(account.dailyLimit),
...
Template['views_send'].onCreated(function() {
  ...
  Helpers.checkChain(function(error) {
    if (error && (HucAccounts.find().count() > 0)) {
  ...
  template.autorun(function(c) {
    var address = TemplateVar.getFrom('.dapp-select-account.send-from',
        'value'),
...
Template['views_send'].onRendered(function() {
  ...
  if (from)
    TemplateVar.setTo('select[name="dapp-select-account"].send-from', 'value',
        FlowRouter.getParam('from').toLowerCase());
...
Template['views_send'].helpers({
  ...
  'hasTokens': function() {
    ...
    return (TemplateVar.get('selectedAction') === 'send-funds' &&
  ...
  'sendAllAmount': function() {
    ...
    if (TemplateVar.get('selectedToken') === 'huc') {
      ...
      amount = (selectedAccount.owners)
          ...
          : BigNumber.max(0, new BigNumber(selectedAccount.balance, 10).minus(
              new BigNumber(gasInWei, 10))).toString(10);
    } else {
   ...
   */
  'timeText': function() {
    return TAPi18n.__('wallet.send.texts.timeTexts.' + ((Number(
  ...
  'formattedCoinBalance': function(e) {
    ...
    return (this.balances && Number(this.balances[selectedAccount._id]) > 0)
        ? Helpers.formatNumberByDecimals(this.balances[selectedAccount._id],
        this.decimals) + ' ' + this.symbol
        : false;
  },
  /**
...
Template['views_send'].events({
  ...
  'keyup input[name="amount"], change input[name="amount"], input input[name="amount"]': function(
    ...
    if (TemplateVar.get('selectedToken') === 'huc') {
      ...
      amount = new BigNumber(amount, 10).times(
          Math.pow(10, token.decimals || 0)).floor().toString(10);
  ...
  'submit form': function(e, template) {
    ...
    var amount = TemplateVar.get('amount') || '0',
        ...
        selectedAccount = Helpers.getAccountByAddress(template.find(
            'select[name="dapp-select-account"].send-from').value),
    ...
    if (selectedAccount && !TemplateVar.get('sending')) {
      ...
      var sendTransaction = function(estimatedGas) {
        ...
        estimatedGas = estimatedGas ||
            Number($('.send-transaction-info input.gas').val());
        console.log('Finally choosen gas', estimatedGas);

        // CONTRACT TX
        if (contracts['ct_' + selectedAccount._id]) {

          contracts['ct_' + selectedAccount._id].execute.sendTransaction(to ||
              '', amount || '', data || '', {
            ...
            if (!error) {
              ...
              addTransactionAfterSend(txHash, amount, selectedAccount.address,
                  to, gasPrice, estimatedGas, data);
      ...
      if (typeof mist === 'undefined') {
        ...
        HucElements.Modal.question({
          ...
          data: {
            ...
            gasPrice: gasPrice,
            estimatedGas: estimatedGas,
            estimatedGasPlusAddition: sendAll ? estimatedGas : estimatedGas +
                100000, // increase the provided gas by 100k
            data: data,
          },
          ok: sendTransaction,
          cancel: true,
