...
Template['views_account_create'].onCreated(function() {
  TemplateVar.set('selectedSection',
      Number(FlowRouter.getQueryParam('ownersNum')) > 0
          ? 'multisig'
          : 'simple');
  ...
  TemplateVar.set('multisigSignatures', Number(
      FlowRouter.getQueryParam('requiredSignatures')) || 2);
...
Template['views_account_create'].helpers({
  ...
  'signees': function() {
    ...
    if (TemplateVar.get('multisigSignatures') >
        TemplateVar.get('multisigSignees')) {
...
Template['views_account_create'].events({
  ...
  'submit': function(e, template) {
    ...
    var deployFrom = template.find('select[name="dapp-select-account"]').
        value.
        toLowerCase();
    ...
    if (type === 'multisig') {
      ...
      var owners = _.uniq(
          _.compact(_.map(template.findAll('input.owners'), function(item) {
            if (webu.isAddress(item.value))
