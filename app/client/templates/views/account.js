...
var addLogWatching = function(newDocument) {
  var contractInstance = webu.huc.contract(newDocument.jsonInterface).
      at(newDocument.address);
  var blockToCheckBack = (newDocument.checkpointBlock || 0) -
      happyucConfig.rollBackBy;

  if (blockToCheckBack < 0)
    blockToCheckBack = 0;

  console.log('EVENT LOG:  Checking Custom Contract Events for ' +
      newDocument.address + ' (_id: ' + newDocument._id + ') from block # ' +
      blockToCheckBack);
  ...
  var filter = contractInstance.allEvents(
      {fromBlock: blockToCheckBack, toBlock: 'latest'});

  // get past logs, to set the new blockNumber
  var currentBlock = HucBlocks.latest.number;
  filter.get(function(error, logs) {
    if (!error) {
      // update last checkpoint block
      CustomContracts.update({_id: newDocument._id}, {
        $set: {
          checkpointBlock: (currentBlock || HucBlocks.latest.number) -
          happyucConfig.rollBackBy,
        },
      });
    }
  ...
  filter.watch(function(error, log) {
    if (!error) {
      var id = Helpers.makeId('log',
          webu.sha3(log.logIndex + 'x' + log.transactionHash + 'x' +
              log.blockHash));
      ...
      if (log.removed) {
        ...
        _.each(log.args, function(value, key) {
          // if bignumber
          if ((_.isObject(value) || value instanceof BigNumber) &&
              value.toFormat) {
        ...
        webu.huc.getBlock(log.blockHash, function(err, block) {
          if (!err) {
...
Template['views_account'].helpers({
  ...
  'jsonInterface': function() {
    return (this.owners) ? _.clone(walletInterface) : _.clone(
        this.jsonInterface);
  },
  /**
  ...
  'availableToday': function() {
    return new BigNumber(this.dailyLimit || '0', 10).minus(
        new BigNumber(this.dailyLimitSpent || '0', '10')).toString(10);
  },
  /**
  ...
  'showDailyLimit': function() {
    return (this.dailyLimit && this.dailyLimit !==
        happyucConfig.dailyLimitDefault);
  },
  /**
  ...
  'formattedTokenBalance': function(e) {
    ...
    return (this.balances && Number(this.balances[account._id]) > 0)
        ? Helpers.formatNumberByDecimals(this.balances[account._id],
  ...
  'ownedAccount': function() {
    return HucAccounts.find({address: this.address.toLowerCase()}).count() > 0;
  },
  /**
  ...
  'displayName': function() {
    return this.ens
        ? this.name.split('.').slice(0, -1).reverse().join(' ▸ ')
        : this.name;
  },
...
Template['views_account'].events({
  ...
  'click button.delete': function(e, template) {
    ...
    HucElements.Modal.question({
      text: new Spacebars.SafeString(TAPi18n.__(
          'wallet.accounts.modal.deleteText') +
  ...
  'click .interface-button': function(e) {
    e.preventDefault();
    var jsonInterface = (this.owners) ? _.clone(walletInterface) : _.clone(
        this.jsonInterface);
