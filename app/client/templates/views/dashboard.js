...
Template['views_dashboard'].helpers({
  ...
  'accounts': function() {
    ...
    var accounts = HucAccounts.find({name: {$exists: true}}, {sort: {name: 1}}).
        fetch();
  ...
  'hasAccounts': function() {
    return (HucAccounts.find().count() > 0);
  },
  /**
   Are there any accounts?
  ...
  'hasMinimumBalance': function() {
    ...
    _.each(_.pluck(HucAccounts.find({}).fetch(), 'balance'), function(bal) {
  ...
  'pendingConfirmations': function() {
    return _.pluck(PendingConfirmations.find(
        {operation: {$exists: true}, confirmedOwners: {$ne: []}}).fetch(),
        '_id');
  },
});
    ...
    mist.requestAccount(function(e, accounts) {
      if (!e) {
        ...
        accounts.forEach(function(account) {
          ...
          HucAccounts.upsert({address: account}, {
            $set: {
              address: account,
              new: true,
            },
