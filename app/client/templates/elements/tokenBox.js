...
Template['elements_tokenBox'].helpers({
  ...
  'formattedTotalBalance': function(e) {
    ...
    var walletsAndAccounts = _.map(
        Wallets.find().fetch().concat(HucAccounts.find().fetch()),
        function(account) {
  ...
  'click .delete-token': function(e) {
    ...
    HucElements.Modal.question({
      text: new Spacebars.SafeString(
          TAPi18n.__('wallet.tokens.deleteToken', {token: token.name})), // could be vulnerable as token name is not HTML purified
      ok: function() {
